...
from app.core.error_handlers import convert_exceptions, raise_database_error
...
async def init_db():
    ...
    with convert_exceptions("database initialization"):
...
async def save_run(run_id: str, graph_name: str, status: str, result: Dict|None, error: str|None):
    """Save run with enhanced error handling."""
    with convert_exceptions("run save"):
        db_path = await get_db_path()
        async with aiosqlite.connect(db_path) as db:
            await db.execute(
...
async def append_event(run_id: str, evt: Dict[str, Any]):
    ...
    with convert_exceptions("event append"):
        db_path = await get_db_path()
        async with aiosqlite.connect(db_path) as db:
            await db.execute(
...
async def load_events(run_id: str) -> List[Dict[str, Any]]:
    ...
    with convert_exceptions("event load"):
        db_path = await get_db_path()
        async with aiosqlite.connect(db_path) as db:
